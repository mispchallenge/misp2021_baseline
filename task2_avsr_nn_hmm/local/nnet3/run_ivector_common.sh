...
if [ $stage -le 1 ]; then
  ...
  steps/make_mfcc_pitch.sh --cmd "$train_cmd" --nj 20 data/${train_set}_sp || exit 1; 
...
if [ $stage -le 6 ]; then
  for data in $test_sets; do
      if [ ! -e data/${data}_hires/.done ];then
        utils/copy_data_dir.sh data/$data data/${data}_hires || exit 1;
        steps/make_mfcc.sh --nj 20 --mfcc-config conf/mfcc_hires.conf \
            --cmd "$train_cmd" data/${data}_hires || exit 1;
        steps/compute_cmvn_stats.sh data/${data}_hires || exit 1;
        utils/fix_data_dir.sh data/${data}_hires || exit 1;
        touch data/${data}_hires/.done
      fi
      steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj ${nj} \
