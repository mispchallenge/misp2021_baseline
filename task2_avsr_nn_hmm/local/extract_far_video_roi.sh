  ...
  if [[ ! -f $video_dir/segment.done ]]; then
    ...
    for n in `seq $nj`; do
      cat <<-EOF > $segment_video_dir/log/roi.$n.sh
        ${python_path}python local/prepare_far_video_roi.py --ji $((n-1)) --nj $nj $video_dir $roi_json_dir $segment_video_dir
EOF
    done
...
if [ $stage -le 2 ]; then
  if [ ! -d $extractor_dir ]; then
    git clone https://github.com/mpc001/Lipreading_using_Temporal_Convolutional_Networks.git $extractor_dir
  fi

  if [[ ! -f $video_dir/extract.done ]]; then
    mkdir -p $visual_embedding_dir/log
    #notice: -g 0 1 2 3 pay attention to your GPU numbers
    ${python_path}python local/prepare_visual_embedding_extractor.py -nj $nj -g 0 1 2 3  -p $python_path $video_dir
...
if [ $stage -le 3 ]; then
  if [ ! -f $video_dir/concatenate.done ]; then
    ...
    ${python_path}python local/concatenate_feature.py --ji 0 --nj 1 $audio_dir $video_dir $av_dir/data
