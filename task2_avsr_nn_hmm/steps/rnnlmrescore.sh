...
if [ $stage -le 7 ]; then
  ...
  for n in `seq $nj`; do
    paste $adir.$n/lmwt.nolm $adir.$n/lmwt.lmonly $adir.$n/lmwt.rnn | awk -v rnnweight=$rnnweight \
     ...
     #score = graphscore+((1-rnnweight)*lmscore);
     #score = graphscore+(rnnweight*lmscore);
     #score = graphscore+(1.9*lmscore)+(rnnweight*rnnscore);
     #score = graphscore+(1.43*lmscore)+(rnnweight*rnnscore);
