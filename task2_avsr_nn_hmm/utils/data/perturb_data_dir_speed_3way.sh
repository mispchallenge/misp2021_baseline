...
if [ $# != 2 ]; then
  ...
  echo "Applies standard 3-way speed perturbation using factors of 0.8, 1.0 and 1.33333."
  echo "e.g.:"
...
utils/data/perturb_data_dir_speed.sh --include-spk-prefix $include_spk_prefix 0.8 ${srcdir} ${destdir}_speed0.8 || exit 1
utils/data/perturb_data_dir_speed.sh --include-spk-prefix $include_spk_prefix 1.33333 ${srcdir} ${destdir}_speed1.33333 || exit 1
...
if $always_include_prefix; then
  ...
  utils/data/combine_data.sh $destdir ${destdir}_speed1.0 ${destdir}_speed0.8 ${destdir}_speed1.33333 || exit 1

  rm -r ${destdir}_speed0.8 ${destdir}_speed1.33333 ${destdir}_speed1.0
else
  utils/data/combine_data.sh $destdir ${srcdir} ${destdir}_speed0.8 ${destdir}_speed1.33333 || exit 1
  rm -r ${destdir}_speed0.8 ${destdir}_speed1.33333
fi
