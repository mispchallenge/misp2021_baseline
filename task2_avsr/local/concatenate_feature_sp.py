...
def main_concatenate(audio_dir, visual_dir, store_dir, ji=None, nj=None):
    ...
    if "train" in audio_dir :
        suffixdic =  dict(zip(["","sp0.8-","sp1.33333-"],[4,5,3]))
    else:
        suffixdic = {"":4}
    for key_idx in tqdm(range(len(common_keys)), leave=True, desc='0' if ji is None else str(ji)):
        ...
        if processing_token:
            ...
            for suffix in suffixdic.keys():
                audio_array = audio_loader[suffix+key]
                expend_visual_array = np.stack(
                    [visual_array for _ in range(suffixdic[suffix])], axis=-1).reshape(-1, visual_array.shape[-1])
                expend_visual_array = expend_visual_array[1:]
                expend_visual_array = expend_visual_array[:audio_array.shape[0]]
                if expend_visual_array.shape[0] != audio_array.shape[0]:
                    print("not in the shape please check")
                    exit(1)
                audio_visual_array = np.concatenate([audio_array, expend_visual_array], axis=-1)
                kaldiio.save_ark(ark=store_ark, array_dict={suffix+key: audio_visual_array}, scp=store_scp, append=True)
    return None
...
if __name__ == '__main__':
    ...
    parser.add_argument('store_dir', type=str, default='data/train_far_sp_hire_av',
