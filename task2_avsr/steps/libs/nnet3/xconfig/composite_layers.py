...
class XconfigTdnnfLayer(XconfigLayerBase):
    ...
    def _generate_config(self):
        ...
        if time_stride != 0 and context != 'none':
            time_offsets1 = '{0},0,{1}'.format(-time_stride,time_stride)
            if context == 'default':
                time_offsets2 = '{0},0,{1}'.format(-time_stride,time_stride)
        ...
        dilation_offsets1 = '-1,0,1'
        dilation_offsets2 = '-2,0,2'
        dilation_offsets3 = '-4,0,4'
        dilation_offsets4 = '-8,0,8'
        ...
        if dilation == 1:
            ...
            output_dim_part = int(output_dim/4)
            configs.append('component name={0}.dilation1 type=TdnnComponent '
                           'input-dim={1} output-dim={2} l2-regularize={3} max-change={4} '
                           'time-offsets={5}'.format(
                               name, bottleneck_dim, output_dim_part, l2_regularize,
                               max_change, dilation_offsets1))
            configs.append('component-node name={0}.dilation1 component={0}.dilation1 '
                           'input={0}.linear'.format(name))
            configs.append('component name={0}.dilation2 type=TdnnComponent '
                           'input-dim={1} output-dim={2} l2-regularize={3} max-change={4} '
                           'time-offsets={5}'.format(
                               name, bottleneck_dim, output_dim_part, l2_regularize,
                               max_change, dilation_offsets2))
            configs.append('component-node name={0}.dilation2 component={0}.dilation2 '
                           'input={0}.linear'.format(name))
            configs.append('component name={0}.dilation3 type=TdnnComponent '
                           'input-dim={1} output-dim={2} l2-regularize={3} max-change={4} '
                           'time-offsets={5}'.format(
                               name, bottleneck_dim, output_dim_part, l2_regularize,
                               max_change, dilation_offsets3))
            configs.append('component-node name={0}.dilation3 component={0}.dilation3 '
                           'input={0}.linear'.format(name))
            configs.append('component name={0}.dilation4 type=TdnnComponent '
                           'input-dim={1} output-dim={2} l2-regularize={3} max-change={4} '
                           'time-offsets={5}'.format(
                               name, bottleneck_dim, output_dim_part, l2_regularize,
                               max_change, dilation_offsets4))
            configs.append('component-node name={0}.dilation4 component={0}.dilation4 '
                           'input={0}.linear'.format(name))
            configs.append('component name={0}.affine type=NoOpComponent '
                           'dim={1}'.format(name, output_dim))
            configs.append('component-node name={0}.affine component={0}.affine '
                           'input=Append({0}.dilation1,{0}.dilation2,{0}.dilation3,{0}.dilation4)'.format(name))
